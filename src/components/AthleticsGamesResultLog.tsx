...
const AthleticsGamesResultLog: React.FC<AthleticsGamesResultLogProps> = ({ event }) => {
        ...
        <div className="dialogue">
          <div className="dialogue-box">
            <div className="dialogue-box-head">
              <h2>
                {formatEventName(event.athleticsEventType)} {event.title} Results
              </h2>
              <div className="dialogue-close" onClick={() => setDialogueOpen(false)}>
                &times;
              </div>
            </div>
              ...
              <table className="excel-like-table" id="result-table">
                <thead>
                  <tr>
                    ...
                    <th>Team</th>
                    <th>
                      {!!event.winner?.participants && !!event.winner?.participants[0].distance ? "Distance" : "Time"}
                    </th>
                  </tr>
                </thead>
                    ...
                    {event.winner.participants.map((p, i) => {
                      ...
                      return (
                        <tr key={i}>
                          ...
                          <td>{p.name}</td>
                          <td>{p.team}</td>
                          <td>{p.distance ? p.distance + "m" : p.time}</td>
                        </tr>
                      );
                    })}
                  </tbody>
