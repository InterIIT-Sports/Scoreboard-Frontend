...
const PastGamesResultsViewer = ({ events }: { events: Event[] }) => {
  return (
    <div className={events.length === 0 ? "wire rightContainer" : "rightContainer"}>
      {events.length === 0 ? (
        <>Past Games Results</>
      ) : (
        <>
...
const ResultLog = ({ event }: { event: Event }) => {
  ...
  if (event.event === EventCatagories.ATHLETICS) return <AthleticsGamesResultLog event={event as AthleticsEvent} />;
  else
    return (
      <div className="resultLog fjalla">
        <span className="winner">{event.winner?.team?.name}</span> won {formatEventName(event.event)}
        {!!(event as TennisEvent)?.matchType ? "(" + (event as TennisEvent)?.matchType + ")" : null} {event.title} |{" "}
        {event.subtitle}, against <span className="loser">{getOppName(event)}</span>
      </div>
    );
};
