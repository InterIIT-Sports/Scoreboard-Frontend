...
const EditScores = () => {
  ...
  const liveAbleEvents = useMemo(
    () => allEvents.filter(e => !e.isCompleted && (e.startTime as number) <= new Date().getTime() + EVENT_START_BUFFER),
    [allEvents]
  );
  ...
  return (
    <div className="usersContainer">
      ...
      <div className="main">
        {!loading ? (
          liveEvents && <section className="liveEvents">{liveEvents.map((event, i) => getEventBox(event, i))}</section>
        ) : (
          <>Loading Events Data...</>
        )}
            ...
            liveAbleEvents.map((event, i) => (
              <div key={i}>
                ...
                {(event as AthleticsEvent).athleticsEventType
                  ? (event as AthleticsEvent).athleticsEventType + " - "
                  : null}
                {event.title} - {new Date(event.startTime).toLocaleDateString("en-GB")} - Start Time:{" "}
                ...
                <button
                  ...
                  onClick={async () => {
                    if (event.isStarted) {
                      if ((event.endTime as number) - EVENT_START_BUFFER > new Date().getTime()) {
